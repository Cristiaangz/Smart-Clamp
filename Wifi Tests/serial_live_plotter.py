...
def makeFig(): #Create a function that makes our desired plot
    ...
    plt.ylim(-1,1)                           #Set limits of second y axis- adjust to readings you are getting
...
while True: # While loop that loops forever
    ...
    test = arduinoString.split()
    print(test)
    if len(test) == 0:
        test.append("1234")
	
    temp = float(test[0])#float(arduinoString)          #Convert first element to floating number and put in temp
    P =    float(test[0])#float(arduinoString)           #Convert second element to floating number and put in P
